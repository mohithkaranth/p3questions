...
import { normalizeAnswer, subjectSchema } from "@/lib/quiz-schema";
...
export async function POST(request: Request, context: RouteContext<"/api/quiz/[subject]/submit">) {
  ...
  try {
    ...
    const results = quiz.questions.map((q) => { const answer = body.data.answers[q.id]; return { id: q.id, userAnswer: answer, isCorrect: q.acceptedAnswers.some((a) => normalizeAnswer(a) === normalizeAnswer(answer)), correctAnswer: q.correctAnswer, explanation: q.explanation }; });
