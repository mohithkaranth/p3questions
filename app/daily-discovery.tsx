...
export default function DailyDiscovery() {
  ...
  useEffect(() => { const controller = new AbortController(); fetch("/api/daily-discovery", { signal: controller.signal }).then(async (response) => { if (!response.ok) throw new Error("Discovery unavailable"); return response.json() as Promise<Discovery>; }).then(setDiscovery).catch((error: unknown) => { if (error instanceof Error && error.name !== "AbortError") console.error(error.message); }); return () => controller.abort(); }, []);
